...
io.on('connection', (socket: Socket) => {
  ...
  socket.on('decision-update', (data: { placeId: string; decision: Decision }) => {
    socket.to(`place-${data.placeId}`).emit('decision-updated', data);
  });

  // Handle commission updates
  socket.on('commission-update', (data: { placeId: string; commission: Commission }) => {
    socket.to(`place-${data.placeId}`).emit('commission-updated', data);
  });

  // Handle document uploads
  socket.on('document-uploaded', (data: { placeId: string; document: Document }) => {
